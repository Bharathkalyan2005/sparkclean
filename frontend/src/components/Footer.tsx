...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="relative" style={{ background: '#050505', borderTop: '1px solid rgba(10,255,230,0.2)' }}>
      {/* Top footer */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="col-span-1">
            ...
            <div className="flex gap-3">
              ...
              <a href="#" className="w-9 h-9 rounded-lg flex items-center justify-center transition-colors"
                style={{ background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.12)' }}>
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          {/* Areas */}
          <div>
            <h4 className="font-syne font-bold mb-4" style={{ color: '#0AFFE6' }}>Service Areas</h4>
            <ul className="space-y-2.5">
              {SERVICE_AREAS.map((area, i) => (
                <li key={i} className="flex items-center gap-2">
                  {area.premium && <span className="text-yellow-400 text-xs">⭐</span>}
                  <Link to={`/book?area=${encodeURIComponent(area.name)}`}
                    className="font-dm text-sm transition-colors text-[#606060] hover:text-[#0AFFE6]"
                    style={area.premium ? { color: 'rgba(255,215,0,0.7)' } : {}}>
                    {area.name}
                  </Link>
                </li>
              ))}
